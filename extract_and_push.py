...
# ----------------------------------------------------------------
# AMP URL Transformers
# ----------------------------------------------------------------
def convert_to_amp(url, company):
    """
    Convert a regular URL to its AMP version based on the company.
    """
        ...
        for i in range(max_rows):
            ...
            for comp, (nota_list, video_list) in companies.items():
                ...
                if i < len(urls):
                    ...
                    if url not in used_urls:
                        # Convert to AMP URL based on company
                        amp_url = convert_to_amp(url, comp)
                        row.extend([today, content_type, amp_url] + [""] * 6)
                        used_urls.add(url)
                        any_url = True
                    else:
                    ...
                    if len(urls) > 0:
                        ...
                        if last_url not in used_urls:
                            # Convert to AMP URL based on company
                            amp_url = convert_to_amp(last_url, comp)
                            row.extend([today, content_type, amp_url] + [""] * 6)
            ...
            for gal_key, galleries in gallery_urls_map.items():
                ...
                if i < len(galleries):
                    url = galleries[i]
                    if url not in used_urls:
                        # Convert to AMP URL - use the "Azteca" format for all gallery URLs
                        amp_url = convert_to_amp(url, company_name)
                        row.extend([today, "img", amp_url] + [""] * 6)
                        used_urls.add(url)
                        any_url = True
                    else:
                        row.extend([""] * 9)
                    ...
                    if len(galleries) > 0:
                        ...
                        if last_url not in used_urls:
                            # Convert to AMP URL
                            amp_url = convert_to_amp(last_url, company_name)
                            row.extend([today, "img", amp_url] + [""] * 6)
                            used_urls.add(last_url)
                            any_url = True
                        else:
